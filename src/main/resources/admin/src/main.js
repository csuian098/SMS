...
/** 侧栏已隐藏模块，禁止通过地址栏直接进入 */
const BLOCKED_PATHS_BY_ROLE = {
	'管理员': ['/zhiweidiaodong', '/jixiaokaohe', '/qingjiashenqing'],
	'admin': ['/zhiweidiaodong', '/jixiaokaohe', '/qingjiashenqing'],
	'人事管理员': ['/jixiaokaohe'],
	'员工': ['/jixiaokaohe'],
}
router.beforeEach((to, from, next) => {
	const role = storage.get('role')
	const blocked = BLOCKED_PATHS_BY_ROLE[role] || []
	if (blocked.indexOf(to.path) >= 0) {
		next({ path: '/' })
		return
	}
	next()
