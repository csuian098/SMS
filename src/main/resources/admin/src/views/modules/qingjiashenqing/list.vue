		...
		<template v-if="showFlag ">
			<el-form class="center-form-pv" :style='{"width":"100%","padding":"0","margin":"0 0 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px 20px 0","boxShadow":"0 3px 3px 0px #095dac","borderRadius":"0px","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","position":"relative"}' >
					<div :style='{"margin":"0 1% 10px 0","display":"flex","alignItems":"center"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"16px","fontWeight":"500","height":"40px"}' class="item-label">请假时间</label>
						<el-date-picker
				...
				<el-row class="actions" :style='{"width":"calc(100% + 8px)","margin":"20px 0 20px -4px","flexWrap":"wrap","justifyContent":"flex-end","display":"flex"}'>
					<el-button class="add" v-if="isAuth('qingjiashenqing','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia16" :style='{"margin":"0 0px","fontSize":"16px","color":"#0d5961","height":"auto"}'></span>
						请假
					</el-button>

					<el-button class="btn18" v-if="isAuth('qingjiashenqing','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
			...
			<div :style='{"border":"1px solid #e9eafc","width":"100%","padding":" 0px 15px 20px","boxShadow":"none","borderRadius":"10px","background":"#fff"}'>
				<el-table class="tables"
					...
					<el-table-column :resizable='true' :sortable='true'
												prop="qingjiayuanyin"
						label="请假原因" show-overflow-tooltip>
						<template slot-scope="scope">
							...
							if(this.shBatchForm.sfsh=='是') {
								for(let x in this.shList){
									await this.$http.post('updateColumn/yuangong/2',{
										...
										csuUpdateColumnValue: '+' + this.shList[x].qingjiatianshu,
										csuConditionColumn: 'gonghao',
										csuConditionColumnValue: this.shList[x].gonghao
									}).then(rs=>{
									})
								}
							}
...
<style lang="scss" scoped>
	...
	.center-form-pv .qingjia-search-daterange /deep/ .el-range-separator {
		flex-shrink: 0;
		min-width: 28px;
		padding: 0 8px;
		box-sizing: border-box;
